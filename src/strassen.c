...
static void free_strassen_matrices(
    void *A11,
    void *A12,
    void *A21,
    void *A22,
 ...
 * Time complexity: T(n) = 7T(n/2) + O(n^2), therefore
 * O(n^log2(7)) ~= O(n^2.8074).
 * This pure implementation intentionally recurses down to n == 1, preserving
 * the theoretical recurrence for experimentation. The additional temporary
 * matrices make the reduced multiplication count visible at the cost of
 * greater heap pressure and extra O(n^2) additions/subtractions per level.
 */
void multiply_strassen(int n, matrix_value_t A[n][n], matrix_value_t B[n][n], matrix_value_t C[n][n]) {
    ...
    if (
        ...
        fprintf(stderr, "Error allocating submatrices for Strassen multiplication\n");
        free_strassen_matrices(
            A11, A12, A21, A22,
            B11, B12, B21, B22,
            C11, C12, C21, C22,
